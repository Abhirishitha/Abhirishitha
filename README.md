# 👋 Hi there, I'm Abhirishitha Naraharisetti!

🎓 B.Tech Computer Science Engineering Student (2027)  
🤖 AI & Machine Learning Enthusiast | 💻 Full Stack Developer | ☁️ MERN Stack Developer  
📍 Andhra Pradesh, India

🌐 **Portfolio:** https://agent-6a5dfdad53b9de0743b--abhirishithaportfolio.netlify.app/  
💼 **LinkedIn:** https://www.linkedin.com/in/abhirishitha-naraharisetti-367aba292/  
💻 **GitHub:** https://github.com/Abhirishitha  
🧩 **LeetCode:** https://leetcode.com/u/Abhirishitha_Naraharisetti/  
📧 **Email:** abhirishithanaraharisetti@gmail.com

---

# 🚀 About Me
Hello! I'm a passionate tech enthusiast with a solid foundation in programming and web development. I'm proficient in Python, Java and have experience with HTML, CSS, React.js, and Node.js. I’m particularly interested in Full Stack Development, Machine Learning and Data Analysis. I enjoy tackling challenges, learning new skills, and working on innovative projects.

---
# 💻 Tech Stack

# 💻 Tech Stack & Skills

**Languages:** Java, Python, JavaScript, C, SQL

**Frontend:** React.js, Next.js, HTML5, CSS3, Tailwind CSS, Framer Motion

**Backend:** Node.js, Express.js, Flask, FastAPI

**Database:** MongoDB, MySQL, MongoDB Atlas

**AI/ML:** TensorFlow, Scikit-learn, Pandas, NumPy, OpenCV, Google Gemini API

**Tools:** Git, GitHub, VS Code, Postman, Vercel, Netlify, Render

**Other Areas:** Data Structures & Algorithms (DSA), Object-Oriented Programming (OOP), DBMS, Operating Systems (OS), REST APIs
---

# 🌟 Featured Projects

| Project | Description |
|----------|-------------|
| 🌱 **LeafLens AI** | AI-powered plant disease detection with TensorFlow, Gemini AI, weather-based disease risk analysis, and treatment recommendations. |
| 🏥 **QueueCure** | Smart hospital queue management system with live tracking, receptionist dashboard, Socket.IO, and MongoDB. |
| 💰 **Finance Tracker** | Personal finance manager with budgeting, dashboards, multi-currency support, analytics, and responsive UI. |
| 💼 **CareerConnect** | Job recommendation platform powered by Adzuna API with resume optimization and career insights. |
| 📈 **Cryptocurrency Price Movement Analysis** | Machine Learning project predicting cryptocurrency prices using regression models and feature engineering. |
| 🤖 **HCARM AI Recommendation System** | Hybrid context-aware recommendation engine using semantic search and transformer embeddings. |
| 🎯 **Focus Tracker** | MERN productivity tracker with Chrome Extension for real-time website usage analytics. |

---

# 🏆 Achievements

- 🌟 GirlScript Summer of Code (GSSoC) Contributor
- 🥇 Elite National Level Open Source Quiz – Top Performer
- 🤖 Top 10 – AI/ML Internship Assessment by Unstop
- 🚀 MedhaSprint & ST&C Challenge – Top Performer
- 💻 LeetCode Weekly Contest 508 – Rank 154
- 🎯 Active participant in hackathons, coding contests, and open-source initiatives

---

# 📜 Certifications

- AWS Academy Cloud Foundations &Data Engineering
- Data Analysis using Python By APSSDC
- Google Skill Cloud Foundations
- NPTEL Affective Computing & Foundations of VR And AR
- HackerRank Java & SQL
- Celonis AI
- MERN Stack By Bodha Soft

---

# 🎯 Currently Exploring

- Artificial Intelligence
- Agentic AI
- MERN Stack Development
- Data Structures & Algorithms
---

# 🤝 Connect With Me

🌐 **[Portfolio](https://agent-6a5dfdad53b9de0743b--abhirishithaportfolio.netlify.app/)** •
💼 **[LinkedIn](https://www.linkedin.com/in/abhirishitha-naraharisetti-367aba292/)** •
💻 **[GitHub](https://github.com/Abhirishitha)** •
🧩 **[LeetCode](https://leetcode.com/u/Abhirishitha_Naraharisetti/)**

> **"Building intelligent solutions that solve real-world problems through AI and Full Stack Development."**

⭐ Thanks for visiting my profile! Feel free to explore my repositories and connect with me.
